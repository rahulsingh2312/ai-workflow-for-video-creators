...
export function heldConditions(proof: RouteProof, performing?: string): Condition[] {
...
> = {
  NEW: {
    ...
    authority_zh: "选题智能体",
    onFail_en: "The run is marked failed and stays visible with its error. No half-list reaches the inbox.",
    onFail_zh: "该次运行标记为失败，带错误信息留在界面上。不会把半份清单丢进收件箱。",
    prove: ({ workspaceId }) => {
      ...
      const withoutSource = cands.filter(
        (x) =>
          (one<{ n: number }>("SELECT count(*) n FROM candidate_source WHERE candidate_id = ?", x.id)?.n ?? 0) === 0,
      );
      const withoutReason = cands.filter((x) => !x.reason_en || x.score == null);
      const banned = JSON.parse(cfg(workspaceId, "topic.banned") ?? "[]") as string[];
      return [
        c("run", "A topic run has produced candidates", "选题运行已产出候选", cands.length > 0,
          `${cands.length} undecided candidates`),
        c("sources", "Every candidate carries at least one source reference", "每条候选至少带一个来源引用",
          cands.length > 0 && withoutSource.length === 0,
          withoutSource.length ? `${withoutSource.length} without a source` : undefined),
        c("reason", "Every candidate carries a score and the reason for it", "每条候选都带分数和理由",
          cands.length > 0 && withoutReason.length === 0),
        c("banned", "Banned topics are filtered out", "禁选清单已过滤", true,
  ...
  TOPIC_REVIEW: {
    ...
    authority_zh: "审核",
    onFail_en: "The candidate holds in review. An undecided topic is not a rejected topic and does not expire on its own.",
    ...
    prove: ({ taskId }) => {
      ...
      const cand = task?.candidate_id
        ? one<{ decision: string | null; decision_reason: string | null; decided_by: string | null }>(
            "SELECT decision,decision_reason,decided_by FROM topic_candidate WHERE id = ?",
            task.candidate_id,
          )
        : null;
      return [
        c("accepted", "A human accepted this candidate", "有人通过了这条候选",
          cand?.decision === "accepted", cand?.decision ?? "no decision"),
        c("reason", "The decision carries a reason", "决定写明了理由", Boolean(cand?.decision_reason)),
        c("who", "The decision records who made it", "决定记录了处理人", Boolean(cand?.decided_by)),
      ];
    },
  },
  ...
  TOPIC_SELECTED: {
    ...
    authority_zh: "脚本智能体",
    onFail_en: "Generation is blocked, not guessed. A missing style profile or source set is reported as the reason.",
    onFail_zh: "生成被拦下，而不是靠猜。缺风格档案或缺来源，会被写成拦下的原因。",
    prove: ({ taskId, workspaceId }) => {
      const srcCount =
        one<{ n: number }>("SELECT count(*) n FROM task_source WHERE task_id = ?", taskId)?.n ?? 0;
      const task = one<{ platforms: string }>("SELECT platforms FROM content_task WHERE id = ?", taskId);
      const platforms = JSON.parse(task?.platforms ?? "[]") as string[];
      const style = cfg(workspaceId, "script.style_profile");
      return [
        c("sources", "The approved source set is attached to the task", "已批准来源已挂到任务上",
          srcCount > 0, `${srcCount} sources attached`),
        c("style", "The client style profile resolves for this workspace", "客户风格档案可解析",
          Boolean(style), style ?? "unset"),
        c("platforms", "Target platforms are set", "目标平台已设定", platforms.length > 0,
          platforms.join(", ") || "none"),
      ];
    },
  },
  ...
  SCRIPT_DRAFT: {
    ...
    authority_zh: "核查智能体",
    onFail_en: "A draft with no claim map cannot enter review. There would be nothing to check it against.",
    onFail_zh: "没有主张对照表的草稿进不了核查。因为没有东西可以拿来对。",
    prove: ({ taskId }) => {
      const v = latestVersion(taskId);
      const claims = v
        ? (one<{ n: number }>("SELECT count(*) n FROM claim_map WHERE version_id = ?", v.id)?.n ?? 0)
        : 0;
      return [
        c("version", "The draft is saved as a version", "草稿已存为版本", Boolean(v), v?.label),
        c("claims", "A claim-to-source map exists for that version", "该版本有主张与来源对照表",
          claims > 0, `${claims} claims mapped`),
        c("count", "The character count is recorded", "字数已记录", (v?.chars ?? 0) > 0, `${v?.chars ?? 0} characters`),
      ];
    },
  },
  ...
  FACT_REVIEW: {
    ...
    authority_zh: "只有审核",
    onFail_en: "The signal stays at danger and the task holds in fact review. One unresolved HIGH flag is enough.",
    onFail_zh: "信号保持禁止，任务留在核查阶段。一条没处理的 HIGH 就够了。",
    prove: ({ taskId, workspaceId }) => {
      const v = latestVersion(taskId);
      const requires = JSON.parse(cfg(workspaceId, "risk.requires_reviewer") ?? '["HIGH","CRITICAL"]') as string[];
      const open = v
        ? all<{ level: string }>(
      ...
      */
      ...
      return [
        c("checked", "The Fact Check Agent has run against this version", "核查智能体已跑过该版本",
          checked, v?.label),
        c("critical", "Every CRITICAL flag has a human resolution", "每条 CRITICAL 都有人处理",
          critical === 0, critical ? `${critical} unresolved` : "none open"),
        c("high", "Every HIGH flag has a human resolution", "每条 HIGH 都有人处理",
          high === 0, high ? `${high} unresolved` : "none open"),
        c("signed", "Every resolution records a reviewer and a time", "每次处理都记下了处理人和时间",
          unsigned === 0, unsigned ? `${unsigned} missing a signature` : undefined),
        /*
  ...
  SCRIPT_LOCKED: {
    ...
    authority_en: "Reviewer",
    authority_zh: "审核",
    onFail_en: "Export is refused without an assigned producer. A locked script nobody holds is how work goes missing.",
    onFail_zh: "没指派制作人员就不给导出。锁了却没人接手的脚本，正是活儿丢掉的方式。",
    prove: ({ taskId }) => {
      ...
      return [
        c("locked", "A locked version exists to export", "存在可导出的锁定版本", Boolean(locked), locked?.label),
        c("producer", "A producer is assigned", "已指派制作人员", Boolean(task?.producer_id), task?.producer_id ?? "unassigned"),
      ];
    },
  },
  ...
  PRODUCTION: {
    ...
    authority_zh: "制作",
    onFail_en: "A video not bound to a locked version is rejected on upload. There is no orphan bucket.",
    ...
    prove: ({ taskId }) => {
      ...
      return [
        c("asset", "A final video is uploaded or linked", "成片已上传或已挂链接", Boolean(asset), asset?.ref),
        c("bound", "That video is bound to the locked script version", "成片绑定到锁定脚本版本",
          Boolean(asset && locked && asset.script_version_id === locked.id),
          asset && locked ? `${asset.script_version_id} vs ${locked.id}` : undefined),
      ];
    },
  },
  ...
  VIDEO_READY: {
    ...
    onFail_en: "Missing platform rules block generation for that platform only. The others still get their package.",
    onFail_zh: "某个平台缺规则，只拦这个平台。其他平台照常出物料。",
    prove: ({ taskId, workspaceId }) => {
      const task = one<{ platforms: string }>("SELECT platforms FROM content_task WHERE id = ?", taskId);
      const platforms = JSON.parse(task?.platforms ?? "[]") as string[];
      const missingRules = platforms.filter((p) => !cfg(workspaceId, `platform.${p}.rules`));
      ...
      return [
        c("platforms", "Target platforms are chosen", "目标平台已选定", platforms.length > 0, platforms.join(", ")),
        c("rules", "Platform rules resolve from configuration", "平台规则可从配置解析",
          missingRules.length === 0, missingRules.length ? `missing: ${missingRules.join(", ")}` : undefined),
        c("generated", "A valid package exists for each platform", "每个平台都有有效物料包",
          platforms.length > 0 && notReady.length === 0,
          notReady.length ? `pending: ${notReady.join(", ")}` : `${ready.length} ready`),
      ];
    },
  },
  ...
  PUBLISH_PACKAGE_READY: {
    ...
    onFail_en: "There is no failure mode here, because there is no automation here. The package waits until someone publishes and comes back to record it.",
    onFail_zh: "这里没有失败模式，因为这里没有自动化。物料包一直等着，直到有人发完回来登记。",
    prove: ({ taskId }) => {
      const recorded = all<{ platform: string; live_url: string | null; account: string | null; published_at: string | null }>(
        "SELECT platform,live_url,account,published_at FROM publish_package WHERE task_id = ? AND status != 'invalid'",
        taskId,
      );
      const done = recorded.filter((r) => r.live_url && r.account && r.published_at);
      return [
        c("account", "The account used is recorded", "记录使用的账号", done.length > 0),
        c("time", "The publish time is recorded", "记录发布时间", done.length > 0),
        c("url", "The live URL is recorded", "记录线上链接", done.length > 0,
          done.length ? `${done.length} of ${recorded.length} platforms recorded` : "nothing recorded yet"),
      ];
    },
  },
  ...
  PUBLISHED_MANUALLY: {
    ...
    authority_zh: "分析智能体",
    onFail_en: "Missing metrics stay missing and are shown as missing. The report does not fill a gap with an estimate.",
    ...
    prove: ({ taskId, workspaceId }) => {
      ...
      return [
        c("imported", "Metrics are imported with a collection method on every row", "指标已导入且每行标明采集方式",
          snaps.length > 0 && snaps.every((s) => Boolean(s.collection_method)),
          `${snaps.length} snapshots`),
        c("reviewed", "Every recommendation has been accepted or rejected", "每条建议都已通过或否决",
          undecided === 0, undecided ? `${undecided} still open` : "all decided"),
      ];
    },
  },
};
...
export function proveRoute(workspaceId: string, taskId: string): RouteProof | null {
...
export function proveAll(workspaceId: string, taskId: string) {
  ...
  return STATES.map((state, i) => {
    ...
    return {
      ...
      manual: Boolean(route?.manual),
      conditions: i <= currentIndex ? conditions : conditions.map((x) => ({ ...x, proved: false })),
      cleared: i < currentIndex || (i === currentIndex && conditions.every((x) => x.proved)),
    };
  });
}
...
export function advance(session: Session, taskId: string, reason?: string) {
  ...
  if (!first) throw new HttpError(404, "no_task", "No such task in this workspace.");

  if (!first.cleared) {
    const held = first.conditions.filter((x) => !x.proved).map((x) => x.label_en);
    audit(session, "transition_refused", "content_task", taskId, { to: first.to }, { held }, reason);
  ...
  */
  ...
  for (let hop = 0; hop < STATES.length; hop++) {
    ...
    run("UPDATE content_task SET state = ?, updated_at = ?, revision = revision + 1 WHERE id = ?",
      proof.to, now(), taskId);
    audit(session, "transition", "content_task", taskId, { state: proof.from }, { state: proof.to }, reason);
    state = proof.to;
  }
  return state;
}
 ...
 */
export function invalidatePackages(session: Session, taskId: string, why: string) {
  const affected = all<{ id: string; platform: string }>(
    "SELECT id,platform FROM publish_package WHERE task_id = ? AND status = 'ready'",
    taskId,
  );
  for (const p of affected) {
    run("UPDATE publish_package SET status = 'invalid', updated_at = ?, revision = revision + 1 WHERE id = ?",
      now(), p.id);
    audit(session, "package_invalidated", "publish_package", p.id, { status: "ready" }, { status: "invalid" }, why);
  }
  if (affected.length) {
    const task = one<{ state: State }>("SELECT state FROM content_task WHERE id = ?", taskId);
    if (task && STATES.indexOf(task.state) >= STATES.indexOf("PUBLISH_PACKAGE_READY")) {
      run("UPDATE content_task SET state = 'VIDEO_READY', updated_at = ?, revision = revision + 1 WHERE id = ?",
        now(), taskId);
      audit(session, "transition", "content_task", taskId, { state: task.state }, { state: "VIDEO_READY" }, why);
    }
  }
