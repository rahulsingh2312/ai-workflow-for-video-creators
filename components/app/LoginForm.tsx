...
const ACCOUNTS: { email: string; name: string; role: { en: string; zh: string }; can: { en: string; zh: string } }[] = [
  {
    ...
    >
      <div className="mx-auto grid w-full max-w-[980px] flex-1 items-center gap-12 px-6 py-12 lg:grid-cols-2 lg:gap-16">
        ...
        <div className="order-2 lg:order-1">
          <span
            className="flex h-11 w-11 items-center justify-center rounded-[14px] text-[16px] font-bold"
          ...
          <form onSubmit={submit} className="mt-8 max-w-sm space-y-4">
            ...
            {error ? (
              <p
                ...
                style={{ background: "var(--bad-bg)", color: "var(--bad)", border: "1px solid var(--bad-line)" }}
              >
                {error}
              </p>
            ) : null}

            <Button type="submit" variant="primary" size="lg" full loading={busy} icon={ArrowRight}>
              {t("Sign in", "登录")}
            </Button>
          </form>
        ...
        <div className="order-1 lg:order-2">
          ...
          >
            <div className="flex items-center gap-2">
              <h2 className="text-[15px] font-semibold">{t("Pick who to be", "选一个身份")}</h2>
              <Badge size="sm">{t("test accounts", "测试账号")}</Badge>
            </div>
                ...
                return (
                  <li key={a.email}>
                    ...
                    >
                      ...
                      {active ? <Check className="h-4 w-4 shrink-0" style={{ color: "var(--ok)" }} /> : null}
                    </button>
                  </li>
                );
              })}
            ...
            <p className="mt-3 text-[12.5px]" style={{ color: "var(--ink-3)" }}>
              {t("They all use the password shown above.", "它们共用上面那个密码。")}
            </p>
          </div>
        </div>
      </div>
