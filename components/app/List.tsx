...
import {
  Circle, CircleCheck, CircleDashed, CircleDot, CircleX,
...
export function List({
  ...
  return (
    <div className={clsx("list", className)} style={{ ["--cols" as string]: columns.join(" ") }}>
      {children}
    </div>
  );
}
...
export function ListCell({ children, className }: { children?: React.ReactNode; className?: string }) {
  return <div className={clsx("flex min-w-0 items-center", className)}>{children}</div>;
}
...
export function ListGroupHeader({
  ...
  return (
    ...
    >
      <span className="text-[14px] font-medium" style={{ color: "var(--ink-gray-8)" }}>
        {label}
      </span>
      <span className="ml-2 text-[13px] tabular-nums" style={{ color: "var(--ink-gray-5)" }}>
        {count}
      </span>
      <span
...
export function Tag({ label, dot }: { label: string; dot?: string }) {
  return (
    <span
      title={label}
      /* Capped and truncated: a tag that clips mid-word reads as a bug, and
         the column has a fixed track so it cannot grow to fit. */
      className="inline-flex min-w-0 max-w-[9rem] shrink-0 items-center gap-1.5 truncate whitespace-nowrap rounded-[var(--r-sm)] px-1.5 py-0.5 text-[12px]"
      style={{ boxShadow: "inset 0 0 0 1px var(--outline-gray-2)", color: "var(--ink-gray-7)" }}
    >
      {dot ? (
...
export function StatusBadge({ label, theme }: { label: string; theme: "red" | "blue" | "green" | "amber" | "gray" }) {
...
export function StatusGlyph({ kind }: { kind: keyof typeof STATUS_ICON }) {
  ...
  return <Icon className="size-4 shrink-0" strokeWidth={1.75} style={{ color: "var(--ink-gray-6)" }} aria-hidden />;
}
...
export function Priority({ level, label }: { level: "high" | "medium" | "low"; label: string }) {
  const p = PRIORITY[level];
  return (
    <span className="flex items-center whitespace-nowrap text-[13px]" style={{ color: "var(--ink-gray-5)" }}>
      <p.icon className="mr-1 size-4 shrink-0" strokeWidth={2} style={{ color: p.color }} aria-hidden />
      {label}
    </span>
  );
}
...
export function CellIcon({ icon: Icon }: { icon: LucideIcon }) {
  return <Icon className="mr-1.5 size-3.5 shrink-0" strokeWidth={1.75} style={{ color: "var(--ink-gray-4)" }} aria-hidden />;
}
